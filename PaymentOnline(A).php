<?php
    ...
    if ($connection->connect_error) {
        ...
        $sql = "SELECT * FROM payment WHERE Payment_Method = 'Online Payment'"; 
...
<body>
    ...
    <h1 style="font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif; color: #000000  ; font-size: 50px; line-height: 1cm;">RESIDENTS ONLINE</h1>
    ...
    <table>
        ...
        <tbody>
            <?php foreach($payments1 as $index => $payment1): ?>
                <tr data-id="<?php echo htmlspecialchars($payment['Payment_Id']); ?>">
                <td><?php echo htmlspecialchars($payment1['Payment_Id']); ?></td>
                    <td><?php echo htmlspecialchars($payment1['Resident_Name']); ?></td>
                    <td><?php echo htmlspecialchars($payment1['Unit']); ?></td>
                    <td><?php echo htmlspecialchars($payment1['Payment_Fees']); ?></td>
                    <td><?php echo htmlspecialchars($payment1['Payment_Method']); ?></td>    
                    <td><?php echo htmlspecialchars($payment1['Payment_Date']); ?></td>
                    <td><?php echo htmlspecialchars($payment1['Payment_Deadline']); ?></td>
                    <td><?php echo htmlspecialchars($payment1['Payment_Status']); ?></td>
                    <td>
                        <?php if ($payment1['Payment_Status'] === 'Approved' || $payment1['Payment_Status'] === 'Rejected' || $payment1['Payment_Status'] === 'Successful' || $payment1['Payment_Status'] === 'Late'): ?>
                        ...
                        <?php else: ?>
                            <form action="PaymentInformationUpdate1.php" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="payment_id" value="<?php echo htmlspecialchars($payment1['Payment_Id']); ?>">
